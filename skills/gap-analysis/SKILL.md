...
| Aspect | Greenfield | Brownfield | Osiris |
|--------|-----------|-----------|--------|
| **Analyzing** | Spec completeness | Existing code vs specs | Spec completeness |
| **Goal** | Validate specs ready to build NEW | Find gaps in CURRENT implementation | Validate specs ready to build NEW |
| **/speckit.analyze** | Skip (no old code to compare) | Run (compare specs to code) | Skip (no old code to compare) |
| **Gap Definition** | Missing requirements, ambiguities | Missing features, partial implementations | Missing requirements, ambiguities |
| **Roadmap** | Build order for NEW app | Fill gaps in EXISTING app | Build order for NEW app |
| **Tech Stack** | ASK user (choosing for new) | Already decided (current stack) | ASK user (choosing for new) |
| **All Features** | ❌ MISSING (building from scratch) | Mix of ✅⚠️❌ (some exist) | ❌ MISSING (building from scratch) |

**Key Insight:**
- **Greenfield/Osiris:** Specs describe WHAT to build (old code doesn't matter)
