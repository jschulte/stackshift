...
## Inspiration: thoth-cli

This workflow is inspired by `thoth-cli`, Cox Automotive's tool for upgrading 90+ Osiris widgets to Node 22 + React 19. We learned from thoth's 3-phase approach:

**What we learned from thoth:**
- ✅ **Phase 0:** Test coverage foundation BEFORE upgrades
- ✅ **Read-only analysis phase:** Plan before executing
- ✅ **Iterative fix loop:** Systematic breaking change resolution
- ✅ **Autonomous test writing:** Don't just analyze, write tests
- ✅ **Comprehensive tracking:** YML files for progress
- ✅ **Validation gates:** Must pass before proceeding

**What we added for specs:**
- ✅ **Spec acceptance criteria = test blueprint**
- ✅ **Spec impact analysis:** Which specs affected by breaking changes
- ✅ **Spec-guided fixes:** Use specs to preserve behavior
- ✅ **/speckit.analyze:** Continuous spec validation
- ✅ **Spec-coverage mapping:** Track which tests validate which specs
...
## Comparison: thoth-cli vs StackShift Modernize

| Aspect | thoth-cli (Osiris) | StackShift Modernize |
|--------|-------------------|---------------------|
| **Target** | 90+ widgets to exact versions | Any app to latest versions |
| **Versions** | Hardcoded (ws-scripts 22, React 19) | Latest stable |
| **Guidance** | Step-by-step commands | **Spec acceptance criteria** |
| **Test Writing** | Coverage-based | **Spec AC-based** |
| **Breaking Fixes** | Trial and error | **Spec-guided** |
| **Validation** | Tests pass | **Tests + /speckit.analyze** |
| **Batch Processing** | Yes (90+ repos) | Single repo |
| **Orchestration** | Go TUI + bash | Claude autonomous |
| **Tracking** | `.osiris-upgrade.yml` | `.stackshift-upgrade.yml` |

**Key Advantage:**
StackShift uses **specs as the source of truth**. When breaking changes occur, specs tell you exactly what behavior to preserve.
